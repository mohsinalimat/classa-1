...
def get_item_price_qty_data(filters):
    ...
    customer_list = frappe.db.sql(
        """select
            ...
            from `tabCustomer` join `tabTerritory` on `tabTerritory`.name = `tabCustomer`.territory
            where `tabTerritory`.branch = '{branch}'
            and `tabCustomer`.disabled = 0
        """.format(branch=branch), as_dict=1)
    ...
    for x in customer_list:
        ...
        item_results = frappe.db.sql("""
                    ...
                    """.format(customer_list=x.customer, from_date=from_date,
