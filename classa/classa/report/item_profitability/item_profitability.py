...
def get_item_price_qty_data(filters):
    ...
    if filters.get("group_by") == "Item":
        item_results = frappe.db.sql("""Select
                ...
                (select ROUND(avg(valuation_rate),2) from tabBin where item_code = b.item_code) as value
                from
        ...
        if item_results:
            for item_dict in item_results:
                data = {
                    ...
                    'net_amount': round(item_dict.net_amount, 2),
                    'cost': round((item_dict.unit * item_dict.value), 2) if item_dict.unit and item_dict.value else 0,
                    'profit': round((item_dict.net_amount - (item_dict.unit * item_dict.value)),
                                    2) if item_dict.net_amount and item_dict.unit and item_dict.value else 0,
                    'cost_percent': round((((item_dict.unit * item_dict.value) / item_dict.net_amount) * 100),
                                          2) if item_dict.unit and item_dict.value and item_dict.net_amount else 0,
                    'profit_percent': round(
                        (((item_dict.net_amount - (item_dict.unit * item_dict.value)) / item_dict.net_amount) * 100),
                        2) if item_dict.net_amount and item_dict.unit and item_dict.value else 0,
                }
                result.append(data)
	...
	# 											{conditions}
	# 				""".format(conditions=conditions, customer=item_dict.customer, tt=filters.get('item_code')),
    #                                         filters, as_dict=1)
    #                 result.append(data)
    #                 for x in details:
    #                     data['unit'] = x.qty
