    ...
    if not filters.get("group_by_item_group"):
        ...
        if item_results:
            for item_dict in item_results:
                ...
                data['p_percent'] = 100 * (pinv1[0][0] / po[0][0]) if po[0][0] else pinv[0][0]
        ...
        for y in item_group_list:
                ...
                opening = frappe.db.sql("""
                                        ...
                                        where
                                            `tabStock Ledger Entry`.item_code = '{itemz}'
                                             ...
                                             and `tabStock Ledger Entry`.is_cancelled = 0
                                        ORDER BY `tabStock Ledger Entry`.posting_date DESC, `tabStock Ledger Entry`.posting_time DESC, `tabStock Ledger Entry`.creation DESC LIMIT 1
                                    """.format(itemz=itemz, item_groupo=item_groupo, from_date=from_date,
                                               warehouseo=warehouseo), as_dict=1)
                for y in opening:
                    opening_qty += y.qty_after_transaction
            data['opening_carton'] = opening_qty / y.carton_cf if y.carton_cf else opening_qty
            ...
            po = frappe.db.sql(""" select 
                                                   ...
                                                   where 
                                                       ...
                                                       and `tabPurchase Order`.transaction_date between '{from_date}' and '{to_date}'
                                               """.format(item_groupo=item_groupo, from_date=from_date, to_date=to_date), as_dict=0)
            ...
            so = frappe.db.sql(""" select 
                                                               ...
                                                               where 
                                                                   ...
                                                                   and `tabSales Order`.transaction_date between '{from_date}' and '{to_date}'
                                                           """.format(item_groupo=item_groupo, from_date=from_date,
                                                                      to_date=to_date), as_dict=0)

            data['so_carton'] = so[0][0]
