...
def get_item_price_qty_data(filters):
    ...
    sp_child_list = frappe.db.sql(
        """select
            (`tabSales Person`.name) as sp
            from `tabSales Person`
            where `tabSales Person`.parent_sales_person = '{sales_person}'
            or `tabSales Person`.name = '{sales_person}'
        """.format(sales_person=sales_person), as_dict=1)

    result = []

    for r in sp_child_list:
        customer_list = frappe.db.sql(
            ...
            """.format(sales_person=r.sp), as_dict=1)
        ...
        for x in customer_list:
            ...
            item_results = frappe.db.sql("""
                        ...
                        """.format(customer_list=x.customer, sales_person=r.sp, from_date=from_date,
